...
if [ "$1" = 'filebeat' ] && [ -e ${DOCKER_SOCK} ]; then
  ...
  processLogs() {
    ...
    # curl --no-buffer -s -XGET --unix-socket ${DOCKER_SOCK} "http://localhost/containers/$CONTAINER/logs?stderr=1&stdout=1&tail=1&follow=1" | tr -d '\000' | tr -d '\001' | sed -e "s/\\\\u0002/[$CONTAINER_NAME] /"  > $PIPE_DIR
    

    curl --no-buffer -s -XGET --unix-socket ${DOCKER_SOCK} "http://localhost/containers/$CONTAINER/logs?stderr=1&stdout=1&tail=1&follow=1" | python -u strip.py $CONTAINER_NAME > $PIPE_DIR
    #curl --no-buffer -s -XGET --unix-socket ${DOCKER_SOCK} "http://localhost/containers/$CONTAINER/logs?stderr=1&stdout=1&tail=1&follow=1" | python -u strip.py | python -u tag.py $CONTAINER_NAME > $PIPE_DIR
